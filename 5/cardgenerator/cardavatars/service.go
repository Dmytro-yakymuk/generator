...
func (service *Service) Generate(ctx context.Context, start int, end int) error {
	...
	for i <= end {
		...
		avatar.Tshirts = rand.SearchValueByPercent(probability.Tshirts5)
		...
		stringComponent := fmt.Sprintf("%d. %v || %v || %v || %v || %v || %v || %v", i, avatar.Background, avatar.Heads, avatar.Tshirts, avatar.Hair, avatar.Headwear, avatar.Glasses, avatar.Earrings)
...
func (service *Service) GenerateNFT(avatar Avatar, sequenceNumber int) error {
	...
	if avatar.Tshirts != "" {
		attributes = append(attributes, Attribute{TraitType: "PUFFA JACKET", Value: avatar.Tshirts})
	}
	if avatar.Hair != "" {
