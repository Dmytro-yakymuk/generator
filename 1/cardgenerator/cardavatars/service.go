...
func (service *Service) Generate(ctx context.Context, cardsTotal int) error {
	...
	for i < cardsTotal {
		...
		if count, err = imageprocessing.LayerComponentsCount(pathToNecklace); err != nil {
			return ErrCardAvatars.Wrap(err)
		}

		if avatar.Necklace, err = rand.RandomInRange(count); err != nil {
			return ErrCardAvatars.Wrap(err)
		}

		if layer, err = imageprocessing.CreateLayer(pathToNecklace, avatar.Necklace); err != nil {
			return ErrCardAvatars.Wrap(err)
		}
		layers = append(layers, layer)
		...
		if count, err = imageprocessing.LayerComponentsCount(pathToJacket); err != nil {
			return ErrCardAvatars.Wrap(err)
		}

		if avatar.Jacket, err = rand.RandomInRange(count); err != nil {
			return ErrCardAvatars.Wrap(err)
		}

		if layer, err = imageprocessing.CreateLayer(pathToJacket, avatar.Jacket); err != nil {
			return ErrCardAvatars.Wrap(err)
		}
		layers = append(layers, layer)
		...
		stringComponent := fmt.Sprintf("%d_%d_%d_%d_%d_%d_%d_%d_%d", avatar.Background, avatar.Heads, avatar.Tshirts, avatar.Jacket, avatar.Necklace, avatar.Hair, avatar.Headwear, avatar.Glasses, avatar.Earrings)
