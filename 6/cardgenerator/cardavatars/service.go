...
func (service *Service) Generate(ctx context.Context, start int, end int) error {
	...
	for i <= end {
		...
		avatar.Hair = rand.SearchValueByPercent(probability.Hairs6)
		...
		avatar.Tshirts = rand.SearchValueByPercent(probability.Tshirts6)
		...
		if rand.IsIncludeRange(service.config.PercentageHeadwear) {
			...
			avatar.Headwear = rand.SearchValueByPercent(probability.Hats6)
		...
		if rand.IsIncludeRange(service.config.PercentageGlasses) {
			...
			avatar.Glasses = rand.SearchValueByPercent(probability.Glasses6)
		...
		stringComponent := fmt.Sprintf("%d. %v || %v || %v || %v || %v || %v", i, avatar.Background, avatar.Heads, avatar.Hair, avatar.Tshirts, avatar.Headwear, avatar.Glasses)
...
func (service *Service) GenerateNFT(avatar Avatar, sequenceNumber int) error {
	...
	if avatar.Tshirts != "" {
		attributes = append(attributes, Attribute{TraitType: "HOOD UP", Value: avatar.Tshirts})
	}
	if avatar.Hair != "" {
